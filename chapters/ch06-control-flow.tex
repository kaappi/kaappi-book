...
Each clause is \texttt{(test expr ...)}. The first clause whose test returns true has its expressions evaluated, and the last one's value is returned. The \texttt{else} clause matches if nothing else did. If no test succeeds and there is no \texttt{else}, the result is unspecified---when you use \texttt{cond} for its value, always include an \texttt{else} clause.
...
Each clause lists datum values in parentheses. The key is compared using \texttt{eqv?}\index{eqv?} (Section~\ref{sec:equality}), which tests equality for numbers, characters, booleans, and symbols (but not strings---two strings with the same content may not be \texttt{eqv?}). For matching against strings, use \texttt{cond} with \texttt{string=?} instead. As with \texttt{cond}, if no clause matches and there is no \texttt{else}, the result is unspecified.
...
A common pattern is using \texttt{or} to provide a fallback when a value may be \texttt{\#f}:
...
Because searching procedures such as \texttt{assoc} and \texttt{member} return \texttt{\#f} on failure, their results chain naturally into \texttt{or}. Note that \texttt{or} falls through only on \texttt{\#f}---unlike in Python, \texttt{0} and \texttt{""} are true values and will not trigger the default.

\subsection{Using \texttt{and} for Guarded Access}

\texttt{and} is useful for guarding an operation that would error on the wrong input---the guard runs first, and the operation runs only if it passes:
...
If \texttt{lst} is empty, \texttt{(car lst)} would raise an error; the \texttt{and} short-circuits to \texttt{\#f} instead. For the lookup-then-extract pattern---testing an \texttt{assoc} result and then taking its \texttt{cdr}---prefer the \texttt{=>} clause of \texttt{cond} shown earlier, which avoids repeating the lookup.

\begin{note}[Coming from Python]
Like Python's \texttt{and}/\texttt{or}, these return the deciding operand rather than a strict boolean, so \texttt{(and x (f x))} works like Python's \texttt{x and f(x)}. The crucial difference is what counts as false: Python treats \texttt{0}, \texttt{""}, and \texttt{[]} as falsy, but Scheme has exactly one false value, \texttt{\#f}. \texttt{(or 0 42)} is \texttt{0}, not \texttt{42}---port idioms like \texttt{val or default} with care.
\end{note}
