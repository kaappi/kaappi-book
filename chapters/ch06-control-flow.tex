...
Each clause lists datum values in parentheses. The key is compared using \texttt{eqv?}\index{eqv?} (Section~\ref{sec:equality}), which tests equality for numbers, characters, booleans, and symbols (but not strings---two strings with the same content may not be \texttt{eqv?}). For matching against strings, use \texttt{cond} with \texttt{string=?} instead.
