...
\section{A Complete Example: SQLite}

Here is a more realistic example that uses SQLite's C API to create a table, insert a row, and query it:

\begin{lstlisting}
(import (kaappi ffi))

(define lib (ffi-open "libsqlite3.dylib"))

;; Bind the functions we need
(define sqlite3-open
  (ffi-fn lib "sqlite3_open" '(string pointer) 'int))
(define sqlite3-close
  (ffi-fn lib "sqlite3_close" '(pointer) 'int))
(define sqlite3-exec
  (ffi-fn lib "sqlite3_exec"
    '(pointer string pointer pointer pointer) 'int))
(define sqlite3-errmsg
  (ffi-fn lib "sqlite3_errmsg" '(pointer) 'string))
\end{lstlisting}

\begin{lstlisting}
;; Open a database (returns a pointer via out-param)
(define db-ptr (make-ffi-pointer))
(sqlite3-open ":memory:" db-ptr)
(define db (ffi-pointer-ref db-ptr))

;; Execute SQL statements
(define (exec sql)
  (let ((rc (sqlite3-exec db sql #f #f #f)))
    (unless (= rc 0)
      (error "SQLite error" (sqlite3-errmsg db)))))

(exec "CREATE TABLE greetings (lang TEXT, msg TEXT)")
(exec "INSERT INTO greetings VALUES ('en', 'Hello')")
(exec "INSERT INTO greetings VALUES ('fi', 'Moi')")
...
\begin{note}
In practice, you would use the \texttt{kaappi-sqlite} package (via \texttt{thottam install kaappi-sqlite}) which wraps these raw FFI calls in a clean Scheme interface. The FFI is for when you need to call a library that does not yet have a Scheme wrapper.
\end{note}
...
In sandbox mode, \texttt{(import (kaappi ffi))} succeeds but all FFI procedures raise an error when called.
...
Test it by binding and calling \code{ceil} from libm:
...
You now know how to call C code from Scheme: open a shared library, bind functions with type signatures, handle strings and pointers, create callbacks, and manage cleanup. In the next chapter, we cover concurrency---fibers, channels, and threads.
