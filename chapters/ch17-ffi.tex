...
On Linux, use the \texttt{.so} name:
...
\begin{itemize}
    ...
    \item \textbf{Pointers:} Opaque pointer values are represented as Scheme FFI pointer objects. You cannot dereference them directly from Scheme.
    \item \textbf{Void:} Use \texttt{'void} as the return type for C functions that return nothing.
\end{itemize}

\section{A Complete Example: SQLite}

Here is a more realistic example that uses SQLite's C API:

\begin{lstlisting}
(import (kaappi ffi))

(define libsqlite (ffi-open "libsqlite3.dylib"))

(define sqlite3-open
  (ffi-fn libsqlite "sqlite3_open" '(string pointer) 'int))
(define sqlite3-close
  (ffi-fn libsqlite "sqlite3_close" '(pointer) 'int))
(define sqlite3-errmsg
  (ffi-fn libsqlite "sqlite3_errmsg" '(pointer) 'string))
\end{lstlisting}
