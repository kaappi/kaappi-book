...
> (+ 1 "two")
; error: type error in 'arithmetic': expected number, got #<string>
\end{lstlisting}
...
\subsection{\texttt{error: vector-ref: index 5 out of range for length 1}}

\textbf{Cause:} \texttt{vector-ref}, \texttt{string-ref}, or \texttt{bytevector-u8-ref} called with an index beyond the length. The message names the procedure, the out-of-range index, and the sequence length.
