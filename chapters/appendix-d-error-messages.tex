...
\begin{itemize}
    ...
    \item The name comes from a \emph{portable} SRFI or an ecosystem library you have not imported. Kaappi's 11 built-in SRFIs (including SRFI-1's \texttt{filter} and \texttt{fold}) are always available, but the 73 portable SRFIs load from \texttt{.sld} files only when imported---\texttt{list-sort} needs \texttt{(import (srfi 132))}. Appendix~\ref{app:srfi} marks which SRFIs are built in.
\end{itemize}
