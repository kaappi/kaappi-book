...
\begin{itemize}
    ...
    \item The name comes from a \emph{portable} SRFI or an ecosystem library you have not imported. Kaappi's \srfibuiltin{} built-in SRFIs (including SRFI-1's \texttt{filter} and \texttt{fold}) are always available, but the \srfiportable{} portable SRFIs load from \texttt{.sld} files only when imported---\texttt{list-sort} needs \texttt{(import (srfi 132))}. Appendix~\ref{app:srfi} marks which SRFIs are built in.
\end{itemize}
...
\textbf{Cause:} A primitive received an argument of the wrong type. The message names the procedure, the expected type, and the offending argument---here, \texttt{car} applied to the empty list. The argument prints readably, so you can see \emph{which} value was wrong: a string appears quoted, a vector with its length.
...
;   expected pair, got ()
> (+ 1 "two")
; error[KP3002]: type error in '+':
;   expected number, got "two"
\end{lstlisting}
...
\subsection{\texttt{error[KP3007]: string-set!: cannot mutate an immutable string}}

\textbf{Cause:} These five mutators reject a value written literally in your source code: \texttt{string-set!}, \texttt{vector-set!}, \texttt{set-car!}, \texttt{set-cdr!}, and \texttt{bytevector-\allowbreak u8-\allowbreak set!}. Strings, vectors, pairs, and bytevectors created by a literal such as \texttt{"abc"}, \texttt{\#(1 2 3)}, \texttt{'(1 2 3)}, or \texttt{\#u8(1 2 3)} are immutable, and so is the string \texttt{symbol->string} returns. This surprises many newcomers, but Kaappi always raises a loud error rather than mutating silently. The code is \texttt{KP3007}, \emph{invalid argument}: the value has the right type but is one the procedure refuses. Pairs and bytevectors report \texttt{immutable pair} and \texttt{immutable bytevector} in the same words:

\begin{lstlisting}[style=repl]
> (define s "abc")
> (string-set! s 0 #\Z)
; error[KP3007]: string-set!: cannot mutate an immutable string
> (vector-set! #(1 2 3) 0 99)
; error[KP3007]: vector-set!: cannot mutate an immutable vector
\end{lstlisting}

\textbf{Fix:} Build a fresh, mutable value instead of mutating the literal: \texttt{string-copy} or \texttt{make-string} for strings, \texttt{vector-copy} or \texttt{make-vector} for vectors, \texttt{list} or \texttt{cons} for pairs.
...
\textbf{Cause:} Non-tail-recursive function with very deep recursion. The call stack grows automatically up to 32768 frames; only recursion deeper than that overflows. The error is not catchable---it unwinds past every \texttt{guard} handler, as does an execution timeout (KP3009).
