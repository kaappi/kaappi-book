...
A raised exception that reaches the top level is reported as \texttt{runtime error: error.ExceptionRaised}. To see the message it carries, catch it with \code{guard}:
...
\subsection{\texttt{runtime error: error.ExceptionRaised}}

\textbf{Cause:} An exception raised by \code{error}, \code{raise}, or a primitive (such as division by zero) reached the top level uncaught.
...
\textbf{Fix:} Wrap the code in \code{guard} to catch the exception and read its message (see Chapter~\ref{ch:errors}). To avoid dividing by zero in the first place, test the divisor: \code{(if (zero? b) ... (/ a b))}.
