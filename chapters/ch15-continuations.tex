...
When Scheme evaluates \texttt{(+ 1 2)}, the continuation is ``take the result and multiply it by 3'':

\begin{center}
\begin{tikzpicture}[
    box/.style={rectangle, draw, rounded corners, font=\ttfamily\small, inner sep=5pt},
    hole/.style={rectangle, draw, dashed, rounded corners, minimum width=1.5em,
                 minimum height=1.2em, font=\ttfamily\small},
    arr/.style={->, >=stealth, thick}
]
\node[box] (expr) {(* 3 (+ 1 2))};
\node[right=2.5cm of expr] (eq) {\Large =};
\node[box, right=0.8cm of eq] (cont) {(* 3 \phantom{XX})};
\node[hole] at ([xshift=0.6em]cont.center) {\textbf{?}};
\node[below=0.3cm of cont, font=\small\itshape] {continuation: (* 3 \rule{1em}{0.4pt})};
\end{tikzpicture}
\end{center}

The dashed box is the ``hole'' where the value of \texttt{(+ 1 2)} will be plugged in. Normally, continuations are implicit---you never see them. Scheme's \texttt{call/cc} makes them explicit.
...
\texttt{call/cc} (short for \texttt{call-with-current-continuation}\index{call-with-current-continuation}) captures the \emph{full} continuation---not just an escape, but a snapshot of the entire execution state that can be invoked multiple times, even after the original computation has finished.
...
A simple coroutine system using continuations. It needs \emph{two} saved continuations: \texttt{return} remembers how to jump back out to the caller, and \texttt{resume} remembers how to jump back into the coroutine:
...
Each call to \texttt{coroutine-run} resumes the coroutine from where it last yielded. The first call starts \texttt{counter} and captures the caller's continuation in \texttt{return}, so \texttt{yield} can escape back out with a value; subsequent calls jump back in through \texttt{resume} and continue after the corresponding \texttt{yield}.

\subsection{Non-Deterministic Choice (Backtracking)}
\index{backtracking}\index{amb}

The classic showpiece for \texttt{call/cc} is nondeterministic choice, known in the Scheme literature as the \texttt{amb} operator. The goal: \texttt{(choose 1 2 3)} returns \texttt{1}---but if the program later calls \texttt{(fail)}, control jumps \emph{back into} that \texttt{choose}, which returns \texttt{2} instead, and everything after it replays with the new value. The program explores alternatives until one works. This is the densest code in the book; read it once, then follow the walkthrough below.
...
\texttt{choose} uses both directions of \texttt{call/cc}, just like the coroutine example. The outer capture grabs \texttt{return}---the continuation of the \texttt{(choose ...)} call itself. Invoking \texttt{(return v)} means ``the value of this \texttt{choose} is \texttt{v}; continue from there.'' The inner capture runs once per alternative and grabs \texttt{k}---the point \emph{inside the loop}, just after the current alternative. Invoking \texttt{(k \#f)} re-enters the loop, which moves on to \texttt{(cdr alts)}.

The two are wired together through \texttt{fail}. Before handing out an alternative with \texttt{(return (car alts))}, \texttt{choose} saves the current \texttt{fail} in \texttt{old-fail} and installs a new one that does two things: restore the previous \texttt{fail}, then jump to \texttt{k} to try the next alternative. So \texttt{fail} is a chain of saved resume points---each pending \texttt{choose} contributes a link, and calling \texttt{(fail)} rewinds to the most recent choice point. When a \texttt{choose} runs out of alternatives, \texttt{fail} has already been restored to the \emph{enclosing} choice point, so the failure propagates outward---and at the top level, to whatever \texttt{fail} was originally: below, a procedure that reports there is no solution.

Note that this genuinely requires full \texttt{call/cc}: \texttt{fail} re-enters \texttt{k} after the \texttt{choose} call has already returned, which is exactly what \texttt{call/ec} forbids.
...
The search tries combinations until \texttt{(+ x y)} equals 8: it backtracks through \texttt{(1 4)}, \texttt{(1 5)}, \texttt{(1 6)}, \texttt{(2 4)}, \texttt{(2 5)}, then finds \texttt{(2 6)}. Follow one step of that trace: \texttt{(1 6)} fails, and \texttt{y}'s \texttt{choose} has no alternatives left, so the failure propagates to \texttt{x}'s choice point. Re-entering it makes \texttt{(choose 1 2 3)} return \texttt{2}, and the body of the \texttt{let} replays from that point---including a \emph{fresh} call to \texttt{(choose 4 5 6)}, which starts over at \texttt{4}.
...
\begin{itemize}
    \item \textbf{Use \texttt{call/ec}} for early returns, loop breaks, and search.\index{non-local exit} This is common and cheap.
...
\begin{itemize}
    \item A continuation captures ``the rest of the computation'' as a first-class value.
    \item \texttt{call/ec} captures an escape continuation for early return and breaking out of loops---efficient and common.
    \item \texttt{call/cc} captures a full, reusable continuation, enabling coroutines and backtracking---powerful but rarely needed.
    \item \texttt{dynamic-wind} runs setup and teardown code even when control jumps into or out of its body.
\end{itemize}
...
Hint: start from the coroutine example in this chapter. You need the same two saved continuations---one to jump out to the caller, one to jump back in---but packaged inside a closure so each generator gets its own pair, plus a flag that flips to \code{'done} when the procedure finishes.
\end{exercise}
