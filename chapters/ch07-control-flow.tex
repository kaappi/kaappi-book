...
\begin{exercise}[FizzBuzz with \code{cond}]
Write a function \code{fizzbuzz} that prints the numbers from 1 to 100, but replaces multiples of 3 with \code{"Fizz"}, multiples of 5 with \code{"Buzz"}, and multiples of both with \code{"FizzBuzz"}. Use \code{cond} for the branching logic and named \code{let} for the loop. Each output should appear on its own line.
...
This is the classic interview question---but in Scheme, \code{cond} makes the intent clearer than a chain of \texttt{if}/\texttt{elif} in Python.
\end{exercise}
...
Rewrite the following \code{do} loop as an equivalent named \code{let}. Both should produce the same result:
