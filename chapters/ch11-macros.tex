...
The failing assertion raises an error object whose message is \texttt{"Assertion failed"} and whose irritant is the \emph{source code} of the assertion---\texttt{'expr} in the template quotes it. The REPL's top-level report is terse, but a handler can read both parts back; Chapter~\ref{ch:errors} shows how.
...
> (assert-equal (* 2 3) 7)
error: assertion failed: (* 2 3) evaluated to 6, expected 7
\end{lstlisting}
\end{exercise}
