...
\section{Procedural Macros with \texttt{er-macro-transformer}}
\label{sec:er-macros}
...
Save the import and the definition as \texttt{strict.scm}, add a good use---\texttt{(display (strict-when (> 3 2) 'ok))}---and then a bad one, \texttt{(strict-when \#t 1 2 3)}. Running the file prints the good result and then stops at the bad use, naming the file, the line and column of that use, and your own message with its irritant:
