...
\texttt{define-record-type} plays the role of a Python \texttt{class} or \texttt{@dataclass}: a named type with named fields, a constructor, and a predicate (like \texttt{isinstance}). Equality is structural, like a \texttt{@dataclass} with its default value equality: \texttt{equal?} answers \texttt{\#t} for two records of the same type whose fields are pairwise \texttt{equal?}, and a record hashes by content too, so it works directly as a hash-table key (Chapter~\ref{ch:stdlib}).\index{records!equality} Identity is still available when you want it: \texttt{eq?} and \texttt{eqv?} test whether two values are the very same object, so they can tell apart two records that \texttt{equal?} considers equal. Because \texttt{member} and \texttt{assoc} compare with \texttt{equal?} by default, they find records by value with no extra argument.
\end{note}
