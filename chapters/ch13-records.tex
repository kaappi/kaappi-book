...
\index{records}\index{define-record-type}\index{data abstraction}
\label{ch:records}
...
\begin{enumerate}
    ...
    \item \textbf{Constructor}\index{constructor} ---
    \texttt{(make-point x y)} specifies the constructor name and
    which fields it initializes.
    \item \textbf{Predicate}\index{predicate!record} --- \texttt{point?} tests whether a value is an instance of this record type.
    \item \textbf{Field specifications} --- each
    field is \texttt{(name accessor)} or
    \texttt{(name accessor mutator)}.
\end{enumerate}

\section{Accessors and Mutators}
\index{accessor}\index{mutator}
...
\index{immutable records}Make fields immutable by default (no mutator). Immutable data is easier to reason about, safe to share between threads, and less prone to bugs. Add mutators only when you genuinely need in-place updates.
\end{note}
...
\texttt{define-record-type} plays the role of a Python \texttt{class} or \texttt{@dataclass}: a named type with named fields, a constructor, and a predicate (like \texttt{isinstance}). One difference to watch for: unlike a \texttt{@dataclass}, two records with identical field values are \emph{not} automatically equal---\texttt{(equal? a b)} is \texttt{\#f} unless \texttt{a} and \texttt{b} are the very same object. Write your own comparison procedure when you need value equality.\index{records!equality} This matters most in lookups. \texttt{member} and \texttt{assoc} accept an equality predicate as an optional last argument---\texttt{(member p points point=?)} finds points by value. SRFI-69 hash tables (Chapter~\ref{ch:stdlib}) offer no such hook: keys always compare with \texttt{equal?}, so a record key matches only the identical object. To look up records by content, key the table with plain data instead, such as a list of field values.
\end{note}
...
(#<<task> "Write chapter" high #f> #<<task> "Review PR" medium #f>)
...
Records print as \texttt{\#<<task> ...>} with their field values in definition order, which makes them easy to inspect at the REPL.
...
(\texttt{current-second}, from the standard \texttt{(scheme time)} library, returns the current time in seconds; Chapter~\ref{ch:stdlib} covers it.)

\section{Building Abstractions with Records}
\index{encapsulation}

Records shine when combined with procedures that form a clean interface. Here is a simple stack:\index{stack}
...
\index{functional update}\index{immutable records}
...
\begin{note}[Coming from Python]
This pattern is like \texttt{dataclasses.\allowbreak replace(obj, age=31)} or
\texttt{NamedTuple.\_replace()}.
Scheme has no built-in syntax for it, so you
...
This is handy at the REPL, but the field names are not shown, the format is not standard across Scheme implementations, and \texttt{read} cannot parse it back. For user-facing output, define a display procedure:
...
\section{Records and Dispatch}

Scheme does not have built-in pattern matching on records (unlike Haskell or Rust). The idiomatic approach is \emph{predicate dispatch}: test with the predicate and destructure with accessors. Consider two shape types:
...
\begin{itemize}
    ...
    \item \textbf{Custom error types} --- Chapter~\ref{ch:errors} uses records to build structured, catchable exception objects.
\end{itemize}
...
\begin{itemize}
    \item \texttt{define-record-type} creates a distinct type with a constructor, a predicate, accessors, and optional mutators.
    \item Records provide named fields and type safety, unlike positional lists.
    \item A constructor need not accept every field, which supports records with defaults.
    \item Functional updates produce new records instead of mutating, enabling immutable designs.
    \item Without built-in pattern matching, records are dispatched on with predicates and destructured with accessors.
\end{itemize}
...
In the next chapter (Chapter~\ref{ch:errors}), we cover error handling---how to signal, catch, and recover from errors.
