...
A fixed-length sequence of bytes (unsigned 8-bit integers). Used for binary data and I/O.
...
A lightweight green thread in Kaappi, cooperatively scheduled within a single OS thread. Created with \texttt{spawn}.
...
An abstract I/O stream. Input ports provide data to read; output ports accept data to write. Backed by files, strings, or bytevectors.
...
Scheme Request for Implementation. A community-standard library specification. Kaappi supports 72 SRFIs.
...
Kaappi's package manager for installing and managing third-party libraries.
