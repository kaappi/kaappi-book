...
The number of arguments a procedure accepts. In the REPL, \texttt{,describe} shows a procedure's arity.
...
An association between an identifier and a value, created by \texttt{define}, \texttt{let}, or a procedure call. Chapter~\ref{ch:bindings} covers local bindings.
...
A fixed-length sequence of bytes (integers 0--255), created with \texttt{bytevector} or \texttt{make-bytevector}. Used for binary data and I/O buffers.

\item[call/cc]\index{call/cc}
Short for \texttt{call-with-current-continuation}. Captures the current continuation as a first-class procedure that can be invoked later (Chapter~\ref{ch:continuations}).
...
A communication primitive for fibers, created with \texttt{make-channel}. A channel is an unbounded queue: \texttt{channel-send} never blocks, while \texttt{channel-receive} waits until a value is available (Chapter~\ref{ch:concurrency}).

\item[closure]\index{closure}
A function bundled with the environment in which it was defined. Closures capture free variables from their enclosing scope (Chapter~\ref{ch:functions}).
...
The ``rest of the computation'' from a given point in the program. Scheme allows capturing continuations as first-class values (Chapter~\ref{ch:continuations}).
...
The R7RS form for creating a library (module). Specifies exports, imports, and the library body (Chapter~\ref{ch:libraries}).

\item[dotted pair]\index{dotted pair}
A pair whose cdr is not a list. Printed as \texttt{(a . b)}.

\item[dynamic extent]\index{dynamic extent}
The period during which control is inside a given expression's evaluation---from entry until exit, including re-entries via continuations. \texttt{dynamic-wind} and \texttt{parameterize} operate on dynamic extents.
...
\item[expander]\index{expander}
The compiler phase that rewrites macro uses into core forms before compilation. Scheme's expander is \emph{hygienic}: expansion cannot accidentally capture user variables.

\item[export]\index{export}
A declaration in a library that makes a binding available to importers. Specified with the \texttt{export} clause in \texttt{define-library} (Chapter~\ref{ch:libraries}).

\item[FFI (Foreign Function Interface)]\index{FFI}\index{Foreign Function Interface}
A mechanism for calling C functions from Scheme. Kaappi provides \texttt{(kaappi ffi)} with \texttt{ffi-open}, \texttt{ffi-fn}, and \texttt{ffi-callback} (Chapter~\ref{ch:ffi}).

\item[fiber]\index{fiber}
A lightweight green thread managed by Kaappi's runtime. Created with \texttt{spawn} from \texttt{(kaappi fibers)}. Fibers are cooperatively scheduled (Chapter~\ref{ch:concurrency}).
...
\item[green thread]\index{green thread}
A thread scheduled by the language runtime rather than the operating system. See \emph{fiber}.

\item[guard clause]\index{guard clause}
An exception-handling form that catches raised exceptions and dispatches on conditions, similar to \texttt{try}/\texttt{except} in Python (Chapter~\ref{ch:errors}).
...
A macro system where generated identifiers cannot accidentally capture variables from the macro's call site. Scheme's \texttt{syntax-rules} is hygienic (Chapter~\ref{ch:macros}).
...
A syntax transformation that rewrites code before it is compiled. Defined with \texttt{define-syntax} and either \texttt{syntax-rules} (pattern-based) or \texttt{er-macro-transformer} (procedural, SRFI-211); see Chapter~\ref{ch:macros}. See also \emph{hygienic macro}.
...
\item[parameter object]\index{parameter object}
A dynamically scoped variable created with \texttt{make-parameter} and rebound for the extent of a body with \texttt{parameterize}. Calling the parameter as a procedure returns its current value.

\item[port]\index{port}
An abstraction for I/O sources and sinks. Input ports read data; output ports write data. Ports can be connected to files, strings, or the console (Chapter~\ref{ch:io}).
...
A user-defined structured data type with named fields. Created with \texttt{define-record-type} (Chapter~\ref{ch:records}).

\item[REPL]\index{REPL}
Read-eval-print loop: the interactive \texttt{kaappi>} prompt that reads an expression, evaluates it, and prints the result. Supports meta-commands prefixed with a comma (Appendix~\ref{app:cli}).
...
A restricted execution mode (\texttt{--sandbox}) for untrusted code. Disables the FFI, subprocesses, file I/O, \texttt{eval}, \texttt{load}, process context access, and OS threads (Appendix~\ref{app:cli}).
...
\item[special form]\index{special form}
An expression evaluated by its own rules rather than as a procedure call---\texttt{if}, \texttt{define}, and \texttt{lambda} are special forms. Appendix~\ref{app:r7rs} lists them all.

\item[SRFI]\index{SRFI}
Scheme Requests for Implementation. Community-designed portable libraries. Kaappi ships \srfitotal{} SRFIs (\srfibuiltin{} built-in, \srfiportable{} portable, three sub-library-only, plus the SRFI 261 naming convention).
...
\item[tail call]\index{tail call}\index{tail position}
A procedure call in \emph{tail position}---the last expression evaluated before returning. Scheme guarantees tail calls run in constant stack space, a property known as tail-call optimization (Chapter~\ref{ch:functions}).

\item[TCO]\index{TCO}
Tail Call Optimization. See \emph{tail call}.
...
\item[truthiness]\index{truthiness}
In Scheme, only \texttt{\#f} is false; every other value---including \texttt{0}, \texttt{""}, and the empty list---is true. A frequent trap for programmers coming from Python or JavaScript.
...
Voluntarily suspends the current fiber so other fibers can run. Implicitly called by \texttt{channel-receive} while it waits for a value.
