...
Write a function \code{count-vowels} that takes a string and returns the number of vowels (a, e, i, o, u---both upper and lower case) it contains. Use \code{string->list} to convert the string to a list of characters, then process the list. You will need \code{char-downcase} and \code{member} (or a series of \code{char=?} checks).
...
Create a vector containing the first 10 perfect squares (1, 4, 9, \ldots, 100) in two different ways:

\begin{enumerate}
\item Using \code{vector-map} on an existing vector of \code{1} through \code{10}.
\item Using a named \code{let} loop that builds the vector with \code{make-vector} and \code{vector-set!}.
\end{enumerate}

Verify that both approaches produce \code{\#(1 4 9 16 25 36 49 64 81 100)}.
\end{exercise}
