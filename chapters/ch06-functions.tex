...
To receive multiple values, use \texttt{call-with-values}\index{call-with-values} or the more convenient \texttt{let-values}\index{let-values} (covered in detail in Chapter~\ref{ch:bindings}):
...
Bonus (after reading Chapter~\ref{ch:hof}): extend \code{my-compose} to accept any number of functions using rest arguments and \code{fold}.
\end{exercise}
