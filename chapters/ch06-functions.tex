...
\begin{exercise}[Function Composition]
Write \code{my-compose} that takes two functions \code{f} and \code{g} and returns a new function that computes \code{(f (g x))}:

\begin{lstlisting}[style=repl]
> (define inc-then-double
    (my-compose (lambda (x) (* x 2))
                (lambda (x) (+ x 1))))
> (inc-then-double 3)
8
> (define shout (my-compose string-upcase
                            (lambda (s) (string-append s "!"))))
> (shout "hello")
"HELLO!"
\end{lstlisting}

Bonus: extend \code{my-compose} to accept any number of functions using rest arguments and \code{fold}.
\end{exercise}
...
The built-in \code{max} already does this, so name yours \code{my-max}. Use a named \code{let} or \code{fold} to walk the rest arguments.
\end{exercise}
