...
\begin{note}[Why prefix notation?]
It is a deliberate trade, not an accident of history. Because the operator is named once, up front, operators can take any number of arguments---infix \texttt{1 + 2 + 3 + 4} names \texttt{+} three times to add four numbers. Because every expression has the same shape, there is no precedence table, and \texttt{+} is not special syntax but an ordinary procedure you can pass around like any other (Chapter~\ref{ch:hof} exploits this constantly). The uniform shape has one more consequence, and it is the big one: Scheme code is itself a Scheme data structure, which is what makes the macros of Chapter~\ref{ch:macros} possible.
\end{note}
...
\begin{note}[Why so strict?]
Python's rule conflates ``no result'' with ``a result that happens to be empty.'' Scheme's rule keeps them apart: a lookup procedure can return \texttt{0}, \texttt{""}, or the empty list as honest \emph{data} and reserve \texttt{\#f} for ``not found,'' and the caller can tell the difference. Older Lisps went the other way---the empty list doubled as false---and Scheme deliberately separated the two. You will see this convention pay off throughout the standard library.
\end{note}
...
The \texttt{'} (single quote) is called \emph{quote}\index{quote}. It prevents the expression that follows from being evaluated. Without it, \texttt{hello} would be looked up as a variable name, causing an ``unbound variable'' error. Quote works on entire lists too---\texttt{'(1 2 3)} builds a list of literal data without evaluating it. Chapter~\ref{ch:lists} covers quoting in depth.
...
\texttt{lambda}\index{lambda} is like Python's \texttt{lambda} keyword, but without the one-expression limitation---a Scheme lambda can contain any number of expressions in its body. For now, remember only that the two definition forms are equivalent; Chapter~\ref{ch:functions} explores \texttt{lambda} and anonymous functions fully.
...
(Scheme already provides this as the built-in \texttt{abs}; we define our own version to show the pattern. Had we called it \texttt{abs}, our definition would silently shadow the built-in one---Scheme lets you redefine anything.)
...
These two forms will carry you through the next few chapters. Chapter~\ref{ch:control-flow} treats conditionals properly---\texttt{cond}'s full clause syntax, \texttt{case}, \texttt{when}, \texttt{unless}, and the boolean combinators.
...
Compare your solution to the \code{my-abs} function defined earlier in this chapter, and to Scheme's built-in \code{abs}. Are they equivalent?
\end{exercise}
...
Translate this logic into Scheme using \code{if}, \code{=}, and \code{-}. If recursion feels unfamiliar, lean on the Python version above and do not worry---Chapter~\ref{ch:functions} is devoted entirely to it.
\end{exercise}
