...
\texttt{error} creates an \emph{error object}\index{error object} and raises it as an exception. The first argument is a message string; additional arguments are \emph{irritants}\index{irritants}---values that provide context about what went wrong.
...
For guaranteed resource cleanup (similar to Python's \texttt{try}/\texttt{finally}), Scheme provides \texttt{dynamic-wind}\index{dynamic-wind}, which we cover in detail in the continuations chapter. For now, the simplest approach is \texttt{call-with-port}, which closes the port when the body returns or raises an exception.
...
For larger programs, you may want to distinguish between different kinds of errors. One approach is to use records as error objects and raise them with \texttt{raise}. We use \texttt{define-record-type} here, which is covered in full in Chapter~\ref{ch:records}---for now, just note the pattern: a type name, a constructor, a predicate, and field specifications:
...
You now know how to signal errors with \texttt{error}, catch them with \texttt{guard}, raise custom conditions with \texttt{raise}, and write robust code that fails gracefully. In the next chapter (Chapter~\ref{ch:records}), we define custom data types with records.
